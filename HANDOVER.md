...
- 已 `git init`，兩個 commit，**沒有 remote**（要推上 GitHub 需先建 repo）
- 追蹤中：`README.md` / `HANDOVER.md` / `docs/PLAN.md` / `.gitignore`
- `tools/` 是空的（PC 端轉檔工具的預留位置，git 不追蹤空資料夾）
- **還沒放 `LICENSE`** —— 已定案 GPL-3.0，但因尚無程式碼而暫緩，補上即可
...
1. **設計字典檔格式** —— 索引結構、內文壓縮、英漢與漢英兩方向如何共存於
   同一檔或分檔。**這是唯一擋住開工的設計問題**，上一段對話一直沒談到。
   已知約束在 PLAN.md §3.3①：索引要固定寬度、對齊 512B 扇區。
2. **決定 D2 / D3 / D4**（PLAN.md §6.2）
