...
**整支韌體編得出 uf2 了（螢幕、鍵盤、SD、狀態機都接上），但一次板子都還沒燒過。**

那不是進度落後，是 PLAN.md §4「後台與前端分離」的直接結果：除了三段硬體
膠水（ILI9341 初始化、矩陣掃描、SD 開檔），所有東西都在 PC 上驗證過 ——
排版逐像素比對、鍵盤與狀態機用腳本跑過完整操作流程。
...
整支 sketch 編出來是 flash 125,524 B（5%）、靜態 RAM 63,312 B（24%）——
RAM 幾乎都是 37.5 KB 的畫布與 SD 函式庫。
...
3. **燒一次板子** —— 這是現在最有價值的一步，因為前面所有東西都已經
