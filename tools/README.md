...
- **真實資料已跑過一輪**（見下表），但只抽驗了少數詞條。異體字、超長片語
  這類邊界情況仍可能有意外。
- **中文的前綴候選仍是字母序**。CC-CEDICT 沒有詞頻欄位，做不了常用詞索引，
  見 FORMAT.md §8。英漢方向已解決。
...
|---|---|---|---|---|---|
| EC（ECDICT） | 770,611 | 24.7 MB + 0.6 MB 常用詞 | 71.4 MB | 12 s | 16 次 |
| CE（CC-CEDICT） | 124,889 | 4.0 MB | 12.1 MB | 1.7 s | 13 次 |

SD 佔用合計約 108 MB。讀取次數與合成資料的預測一致。

前綴候選（FORMAT.md §8）：打 `hel` 在常用詞優先模式下得到
...
產出 `assets/RetroDict.ino.RAW`：96×96 RGB565 **big-endian**、無標頭、
**固定 18432 bytes**。載入器沒有標頭可以驗，只用檔案長度擋「拖錯檔案」。

轉檔本身呼叫載入器自己那支 `make_thumb.py`（規格的正本），但轉完會**把 .RAW
解回圖再驗一次**：長度、尺寸、以及位元組順序真的是 big-endian。順序錯了圖
不會壞成馬賽克、只會整張變色，所以驗的方式是「兩種順序解出來哪一種比較接近
來源圖」，不是「看起來像不像圖」。這把尺本身也試過：把位元組對調、把長度砍掉
2 bytes，兩種都會被抓出來。

來源圖是寬幅的，會先置中裁成正方形（存回 `assets/retrodict_icon.png`，上限
768px）再交給 `--fit`，直接 `--fit` 會把書的上下邊緣切掉。
`assets/retrodict_icon_96.png` 是放大四倍的對照圖，用來看縮到 96px 之後
還讀不讀得出來。
